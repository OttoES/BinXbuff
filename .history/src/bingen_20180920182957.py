...
structAnno          = ZeroOrMore(AT+Group(IDENT("annoName") + EQ + INT("annoValue")))("anno")
#structLocals        = Group(ZeroOrMore(LESS+IDENT("defName") + EQ + INT("defValue") + LARGER))("localConst")
structLocals        = ZeroOrMore(LESS+Group(IDENT + EQ + INT) + LARGER)
structAdds          = structAnno + structLocals("localConst")
...
class BaseCodeGenerator:
    ...
    #s += self.makeClassMemberList(structt)
        ...
        if "parentName" in structt:
            ...
            #s += "(" + self.packBuffName + ", pos, "+ self.genArg(parentStruct) +");\n"
        locConst = structt["localConst"]    
        for ll in locConst:
            s += self.makeConsVarDecl("int",ll['defName'])
