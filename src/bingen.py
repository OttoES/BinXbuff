...
from pyparsing import (Word, alphas, alphanums, Regex, Suppress, Forward,
    Group, oneOf, ZeroOrMore, Optional, delimitedList, Keyword, cStyleComment, pythonStyleComment,
    restOfLine, quotedString, Dict)

comment       = '## ' + restOfLine
#comment       = '#' + restOfLine
CMNT          = cStyleComment("comment")
#CMNT2         = cppStyleComment("comment")
...
enumDefn            = Optional(CMNT("comment")) + ENUM_("typespec") - ident('name') + LBRACE + Dict( ZeroOrMore( Group(ident + EQ + integer + SEMI) ))('values') + RBRACE
