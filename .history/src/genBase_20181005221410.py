...
class BaseCodeGenerator:
    ...
    def makeCallFunArgListComplete(self,structt):
    ...
    def genProcessMsgDetail(self,structt):
        ...
        s += self.makeCallFunArgListComplete(structt)
        s += ");\n"
        return s

    def genSelecAndProcessMsgFun(self):
        s  = "\n// First determine the struct/message type basedon MSG_ID and\n// MSG_COND and then unpck\n"
        s += self.intTypeName + " " + "parseMsg(uint8_t buff[],int len) \n{\n"
    ...
    def genAll(self):
        ...
        for st in structList:
            s += self.genPackFun( st) +"\n"
        s += self.genSelecAndProcessMsgFun()
        return s
