...
class BaseCodeGenerator:
    ...
    def getParent(self,structt):
        if not "parentName" in struct: return None 
    ...
    def genTypedArg(self,fields):    
        ...
        return s[:-1] 
    #####-------------------------------------
    def makeFunName(structt,retType,funName,args,isClassFun = True):
        s = ""
        if isClassFun: s+= "static "
        s += retType + " " + funName + "(" + arg + ")"
        return s
    def makeDeclFunArgList(self,structt):
        self.makeFieldListGeneric(structt,True, seperator = "," , inclParent = True , inclConst = False )    
    def makeCallFunArgList(self,structt):
        self.makeFieldListGeneric(structt,False, seperator = "," , inclParent = True , inclConst = False )
    def makeClassMemberList(self,structt):
        self.makeFieldListGeneric(structt,True, seperator = ";\n  " , inclParent = False , inclConst = True )
    def makeFieldListGeneric(self,structt,inclTypes, seperator = "," , inclParent = True , inclConst = False ):
        """ Flexible generation of varable list 
        """
        fields    = structt["body"]
        s = ""
        if inclParent and "parentName" in structt:
            parnt = self.getParent(structt)
            s += self.genArgs2(parnt,inclTypes) + seperator
        for f in fields:
    ...
    def xxgenPackFun(self,struct,copyToBuff=True,msgIdArg = False):
        ...
        if copyToBuff:
            funName   = self.funcPackBuffPrefix + structnme.capitalize()
        ...
        if "parentName" in struct:
            ...
            s += "    pos = " +self.funcPackBuffPrefix + struct["parentName"].capitalize() 
    ...
    def genPackFun(self,structt,copyToBuff=True,msgIdArg = False):
        ...
        if copyToBuff:
            ...
            s    = self.makeFunName(structt,self.intTypeName,self.funcPackBuffPrefix,args)
            # s  = self.intTypeName +" "+ funName +"(" + parentArgs
        ...
        s += self.genTypedArg(fields) +")\n"
        s += "{\n"
        (structLen,__) = self.genPackLenCalc(struct)
        s += "    const "+self.intTypeName +" "+self.packBuffName+"Len = " + structLen
        #s += "    const int "+self.packBuffName+"Len = " + self.genPackLenCalc(fields)
        s += ";\n"
        if not copyToBuff:
