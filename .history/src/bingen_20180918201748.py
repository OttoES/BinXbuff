...
structDefn          = (CMNT + STRUCT_ - IDENT + Optional(HEADEDBY_ + IDENT("parentName").setParseAction(addToHeaderDict)) + Optional(EXTENDS_ + IDENT("baseName")) +Optional(IDENT + EQ + ANNOTSTR("anno")) + LBRACE + structtBody("body") + RBRACE).setParseAction(addStructToList)
...
struct GGheader
{
  ...
  MSGID16   MSG_ID; // This is to test a very long comment line. Abcdefghijklmnopqrstuvwxyz abcdefghijklmnopqrstuvwxyz
...
class BaseCodeGenerator:
    ...
    def genVarOnlyDecl(self,varType,varName,arrayLen = None):
        ''' Generate variable declaration of the given type.
            If an array size given it will be decalred as an 
            array of that size.
    ...
    def simplifyEq(self,eqs):    
        ...
        ll = eval(eqs)
        return ll,True
    def genPackFun(self,struct,copyToBuff=True,msgIdArg = False):
